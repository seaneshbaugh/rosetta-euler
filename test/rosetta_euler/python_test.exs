...
defmodule RosettaEuler.PythonTest do
  ...
  def run_python_solution(number) do
    name = number |> to_string |> String.pad_leading(3, "0")

    System.cmd("python", ["python/" <> name <> "/" <> name <> ".py"])
  end

  rosetta_euler_tests 1..16 do
    @tag test_number: test_number
    test "Python #" <> to_string(test_number), %{test_number: test_number} do
      answer = answer_for(test_number) <> "\n"

      {^answer, 0} = run_python_solution(test_number)
    end
  end
end
