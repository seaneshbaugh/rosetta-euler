...
defmodule RosettaEuler.CitrineTest do
  ...
  def run_citrine_solution(number) do
    name = number |> to_string |> String.pad_leading(3, "0")

    System.cmd("ctr", ["citrine/" <> name <> "/" <> name <> ".ctr"])
  end

  rosetta_euler_tests 1..1 do
    @tag test_number: test_number
    test "Citrine #" <> to_string(test_number), %{test_number: test_number} do
      answer = answer_for(test_number) <> "\n"

      {^answer, 0} = run_citrine_solution(test_number)
    end
  end
end
