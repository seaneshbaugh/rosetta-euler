defmodule Euler do
  ...
  def list_overlap(a, b) do
    Enum.sort(List.concat(a, list_overlap(:delete, a, b)))
  end
...
IO.puts 1..20 |> Enum.map(fn x -> Euler.prime_factors(x) end) |> Enum.reduce([], fn (x, sum) -> Euler.list_overlap(x, sum) end) |> List.flatten() |> Enum.reduce(1, fn (x, product) -> x * product end)
